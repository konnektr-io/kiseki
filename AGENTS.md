...
| POST | `/api/trips/{trip_id}/crew` | add placeholder person (they claim later). With `sub`: attach an account that already exists — no placeholder, no invite link; owner-only + only someone the caller follows. With `personId`: LINK an unclaimed placeholder already crew on another trip the caller owns — one Person twin, so one claim lands them on every linked trip; owner-only, id-based (never name-matched), `contact` refused |
| GET | `/api/trips/{trip_id}/crew/placeholders` | owner-only: the placeholders this trip may link (`personId` values for the line above), each with the other trips they are already on (#322) |
| DELETE | `/api/trips/{trip_id}/crew/{person_id}` | owner-only; a placeholder Person is deleted only when this trip held its last `hasCrew` edge (#322) |
...
- **The traveler's own position (#383)** — the trip map can show where the *viewer* is, and hand it to the chat. Permission-first by construction: `lib/geolocation.ts` is the only module that touches `navigator.geolocation`, and `lib/device-location.ts` (a module store, not React state) is the only caller. Tracking starts in exactly two ways — the browser **already grants** geolocation for this origin (a returning traveler gets their dot with no prompt), or the traveler **taps the locate control**. A `prompt`/`unknown` permission never starts on its own: a prompt on load trains people to deny, and a denial is sticky. Scope is the **trip map only** (`RouteMap`) — the landing and home maps never call in. The dot is a DOM marker in `--map-locate` (accent, never the trip's signature colour, and no ordinal — it must not read as a place pin or a stage badge) with a real accuracy halo (a `circle` layer whose radius is metres converted at the current zoom, `metersPerPixel`); the control's three states (start / recentre / stop) are a pure table in `locateControl`. Nothing is persisted (no localStorage, no cookie, no coordinate in any analytics event), and the session dies with the page. While it is tracking, a submitted chat turn carries `deviceLocation: {lat, lng, accuracy?}`; the relay phrases it into the turn's instructions (`device_location_line`) so "somewhere to eat around here" resolves to where the traveler IS rather than to the day's stop. That value is the ONE piece of context the relay cannot resolve from the graph (it is client-supplied), so it is a **strict** bounded model — numbers only, `extra="forbid"` — and never a string: nothing a browser sends may reach the agent's system prompt. Verified in a real browser at phone width by `frontend/scripts/probe-device-location.py` (Playwright mocks the permission and the position; the probe counts the app's own `watchPosition` calls, so "never asks on load" is measured, not asserted).
...
- Auth0 tenant `dev-zv5urb33g0msy7bc.eu.auth0.com`, reached through the **custom domain `auth.konnektr.io`** — that is the host the SPA is configured with and the `iss` the backend validates, so `AUTH0_DOMAIN` (frontend baked default + backend env) must be the custom domain, never the raw tenant host. App client `jbMyX3scNHkECOF1lNJTOovXe8fOBmiq` (SPA; Refresh Token Rotation on). Allowed Callback/Logout URLs + Web Origins list the **app** origins only (`https://kiseki.konnektr.io`, `http://localhost:5173`) — the custom domain is an issuer, not a callback. Login-page look, copy and the background live in the Auth0 dashboard; see `docs/auth0-branding.md`.
...
- **Public vs private trips (#64)** — `visibility` on the Trip twin: `public` serves anonymously at `GET /api/trips/{id}`, `private` requires a crew `hasCrew` edge. `discoverable` (#196, default flipped to `True` in #228) is an ADDITIVE listing flag (owner-only): a discoverable `private` trip may be listed on crew profiles / the follower feed but still requires follower+ to read. Join links carry `claimToken` (copy of the trip's `claimToken`) to let a new user `follow` as `follower` or claim a `Person` placeholder. The graph is the only store; there is no `token` flag and no file fallback.
- Placeholder→real-user migration (#6, `app/claims.py`): a **claim token** (separate secret, sibling of `token`) authorizes claiming a crew identity. `GET /join/<claimToken>` shows the trip + crew; `POST /api/claims {claimToken, personId}` creates the User twin (`$dtId` = auth sub) + transfers the `hasCrew` edge (same role/index) + **deletes the placeholder**. No name/email matching — the user picks the person explicitly.
  - **Shared placeholders + the claim cascade (#322)**: a placeholder is ONE `Person` twin with one `hasCrew` edge per trip, so the same unregistered person added to several trips stays one identity with one invite. Adding them to a second trip reuses that twin (`personId` on `POST /crew`, or the owner's picker at `GET /crew/placeholders`); claiming through ANY of the trips' join links transfers **every** edge (each trip keeping its own role/index/note/label) and then retires the placeholder — edges first, twin last (#89 rule). The invite still only authorizes the person ON THE TRIP IT NAMES; the cascade follows that person outward to their other trips, never the other way. Reusing/linking a placeholder is OWNER-only on both sides (it decides access for both trips and hands out no access itself until claimed). A placeholder is deleted only when its LAST `hasCrew` edge goes — `remove_crew` and `delete_trip` both check first, or the twin delete would be refused by the graph (a vertex with edges) on an already-swept trip.
  ...
  - **Adding a TOP-LEVEL route means editing two files**: a new SPA route must be registered in `frontend/src/App.tsx` **and** in the backend's history-mode whitelist (`is_spa_route` in `app/main.py`). Miss the second and in-app navigation works while a deep link or a reload 404s as JSON — that is live in v0.29.0 for `/me` and `/u/:sub`. A child of `/t/:tripId` needs **only** `App.tsx`: the whitelist matches the `t/` prefix, so `/t/<id>/settings` (and any future child) is covered; verified with `TestClient` for `/t/<id>/settings` in `test_spa_fallback_196.py` (#248). The SPA tests only run when `KISEKI_STATIC_DIR` points at a built SPA (CI sets `../frontend/dist`), otherwise they silently skip.
...
3. In `home-k8s`: edit `konnektr/kiseki/deployment.yaml` image tag (**with the `v` prefix**, e.g. `v0.79.2` — the workflow's semver patterns publish `v{{version}}`, `v{{major}}.{{minor}}`, `v{{major}}`).
