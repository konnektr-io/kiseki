...
def render() -> tuple[str, list[str]]:
    ...
    add("")
    add("- Credentials ride in the `Authorization` header (JWT) or `X-API-Key`")
    add("  (admin key, #324) with mandatory per-request `X-Act-As-Sub`; the client")
    add("  reads either credential from `KISEKI_API_KEY` first, then `KISEKI_TOKEN`,")
    add("  and the acting user from `--act-as` / `KISEKI_ACT_AS_SUB`.")
