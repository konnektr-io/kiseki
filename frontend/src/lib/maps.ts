 ...
 * Returns null when the trip has no share token, maps are unconfigured, or the
 * request fails; callers degrade rather than blow up.
 */
export async function fetchRouteLegs(
  ...
  if (!trip.token) return null;
  const resolvable = places.filter((p) => findLocation(trip, p));
  if (resolvable.length < 2) return null;
  const url = `/api/maps/route/${trip.token}?places=${encodeURIComponent(resolvable.join(","))}${loop ? "&loop=1" : ""}`;
  try {
