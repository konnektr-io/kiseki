...
import { SnowStrip } from "./WeatherSnow";
...
export function PlaceFacts({
  ...
  // Snow strip (#334) needs only coords — unlike the Google overlay it MAY
  ...
  const snow =
    place.lat != null && place.lng != null ? <SnowStrip lat={place.lat} lng={place.lng} /> : null;
  if (!placeHasFacts(place) && !hasLive) return snow;
  ...
  return (
    <div className={`${place.photo ? "" : "no-print "}mt-2 space-y-1.5`}>
      ...
      <div className="no-print space-y-1.5">
        ...
        {place.lat != null && place.lng != null && <SnowStrip lat={place.lat} lng={place.lng} />}
