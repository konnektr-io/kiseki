...
import { fetchTricountSnapshot, disconnectTricount, TripAccessError } from "../lib/api";
...
export function TricountPanel() {
  ...
  if (!config) {
    // #231: there is no "not linked yet" affordance here any more. An
    // unlinked trip used to get a prominent setup card on the practical page;
    // that is now an owner-only row in the trip actions menu
    // (`components/trip-controls.tsx`), beside the other trip-level settings.
    // This panel only ever renders for a trip that HAS a connection — the
    // practical page gates on `practical.tricount`, and crew-only means
    // followers/anonymous see nothing either way.
    return null;
  }
