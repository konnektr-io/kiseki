...
- **Media (✅ shipped, #47)**: images live in the **Garage bucket** (S3-compatible, private, key prefix `media/`), served by the backend at relative `/media/<trip>/<file>` URLs (no base64 ever). Keys are content-addressed (`sha256[:32].ext`) so they are unguessable and migration is idempotent; the proxy at the old URL shape means the model stays URL-based — swapping the backend changed only where the bytes live, nothing in the data.
