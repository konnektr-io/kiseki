...
- **Identity**: the agent has no identity in the graph (AGENTS.md → identity model). Interim single-user **TEMPORARY pin**: the content profile mints the M2M token and the backend `KISEKI_AGENT_ACT_AS` (static env) resolves writes as Niko. This pin must never become multi-user plumbing. **Status 2026-09-10**: per-request identity landed for the chat path — the **UI passes the end user's token per request** (mode 1, live) or the M2M token + a request-scoped act-as sub. The pin survives only because envelope-less callers (the content agent's CLI writes) have no act-as channel yet; it goes when the write routes resolve a request-scoped act-as sub. Never provision agent twins/edges, never store a personal user token in a profile.
- **User learning** (pivot 2026-09-09, per Niko): per-user memory uses **Honcho, scoped by `X-Hermes-Session-Key`** — the chat relay sends `<sub>` as the session key, and Honcho's session-key fallback gives each user an independent peer (`user-default-<sub>`) with per-user sessions/representation/memory (verified in the honcho plugin: `_resolve_user_peer_id` falls back to the session key when the platform sets no runtime user id, which the api server never does). Honcho is already deployed (ns `hermes`) and already the home profile's memory provider, so this is cheaper than graph memory nodes. **Graph memory (#10) is demoted to optional/later** — revisit only if Honcho's per-user recall proves insufficient (e.g. per-trip scoping on top). **Live state (verified 2026-09-10)** — peers in workspace `kiseki`: `user-default-google-oauth2|100613034256980569871` (Niko; 52-message session + derived summary), `kiseki-agent`, `hermes_kiseki`. **Mechanism correction (2026-09-10)**: non-chat sessions are *not* peerless — the plugin's `resolve_session_name` derives a session name for them too (gateway session key → session-title slug → cwd basename), so desktop/CLI live probes created `user-default-Bot-Chat` and `user-default-data`. Isolation comes from that derived name never being a user's `sub` (plus the recall skill's `session_key == acting sub` filter); those non-user peers hold only unattributed agent/probe content, and Honcho v3 has no peer DELETE, so they are inert but permanent.
- **Agent memory hygiene**: the content profile's built-in memory holds **no user facts** — only impersonal operational knowledge (endpoints, content conventions). Per-user facts live in **Honcho under the acting user's peer** (session-key scoped); the profile has enabled `memory.provider: honcho` since 2026-09-10 (workspace `kiseki`, aiPeer `kiseki-agent`), so each acting sub gets its own peer. Cross-user isolation is a hard requirement — never prompt discipline alone — but state it honestly: the automated test today covers conversation *naming* (`test_chat_conversation_isolation_between_users`); the Honcho peer isolation rests on the session-key seam + live peer evidence, and a two-live-user check / recorded cross-user-recall negative are **still unprobed**.
...
- **GDPR** (parked until public — trusted group now): EU data residency (home cluster), DPAs with processors, erasure incl. memory, portability = the booklet export, consent for data sources, AI Act transparency. ⚠️ Provider choice (DeepSeek/OpenRouter) is a question *only* when the product goes public — models can be swapped later. **Status (#196 phase C)**: graph-level erasure (`DELETE /api/me` — crew rows revert to placeholders, follows dropped both ways, `User` twin deleted last; 409 while the caller still owns a trip) and portability (`GET /api/me/export` — one JSON document: profile, owned trip docs, own crew rows, social graph) are live. Honcho-peer erasure is still owed (Honcho v3 has no peer DELETE — agent/probe peers are inert but permanent).
...
- **Dynamic maps from `trip.locations`**: Google Maps JS in the web app (markers numbered by the trip's own location order, per-leg DirectionsRenderer routes, live traffic, live drive-time chip), server-side static-map proxy for the PDF (real routes via Directions API encoded polylines, geocoded venue pins for hotels/restaurants from the trip's `locations` registry — see the `kiseki-trip-content` skill for the styled-marker/raw-polyline quirks). Loop maps close back to the start.
...
- [x] **Agent backend + chat UI (#9)** — ✅ **shipped, closed 2026-09-10**: content profile live (§8); `/api/chat` + `/api/files` relay (PR #145); SPA chat + streaming (PRs #147/#148/#153; wire v1 #149/#150; reconnect #155; activity feed #151/#154/#157/#169/#176); `GET /api/trips` honours the resolved identity (#142/#143); handoff loop live (#140). Per-user auth is bearer-first and **mode 1 (the SPA passes the end user's own token) is live**. Shipped as `@ai-sdk/react` `useChat` + `DefaultChatTransport`, not "Vercel ai-elements". **Residual**: the static `KISEKI_AGENT_ACT_AS` pin is still deployed and still load-bearing for envelope-less callers (content-agent CLI writes) — see §8.
- [x] **Agent memory per user (#10 — re-scoped to Honcho 2026-09-09, adopted 2026-09-10)** — ✅ `memory.provider: honcho` enabled on the `kiseki` profile; per-user peers derive from `X-Hermes-Session-Key` and are **live-verified** (a distinct peer per session key in workspace `kiseki`). Graph memory nodes + pgvector stay **demoted to optional** — revisit only if Honcho per-user recall proves insufficient (e.g. per-trip scoping on top). Verification still owed (recorded, not claimed): a two-live-user peer check + a recorded cross-user-recall negative.
- [x] **Agent fleet ops (#140)** — ✅ live (#140 closed): content-agent code requests #157/#158/#160/#163/#171/#172/#175 all shipped through the loop; code→content skill updates ride every API-changing release.
...
| 16 | Content-profile memory | ✅ Built-in agent memory holds **no user facts**; per-user memory = **Honcho peer per `X-Hermes-Session-Key`** (#10 re-scoped 2026-09-09; **adopted + live-verified 2026-09-10**: `provider: honcho` enabled, a distinct peer per session key in workspace `kiseki`) |
...
### Round 3 (2026-09-10) — agent line closed out (issues #9/#10)

| # | Decision | Outcome |
|---|---|---|
| 19 | Agent identity (#9) | ✅ Shipped as built: the SPA passes the **end user's token per request (mode 1)**; #9 closed. The static `KISEKI_AGENT_ACT_AS` pin is **deliberately retained** as the envelope-less fallback (the content agent's `api_write.py` CLI sends no act-as). Removing it is blocked on write-route act-as resolution — only `/api/chat` + `/api/files` accept one today (`acl.resolve_request_actor_sub`); deleting it first would flip unattended writes to mode 3 (owner-level service principal) |
| 20 | Per-user memory (#10) | ✅ Honcho adopted: `memory.provider: honcho` on the `kiseki` profile, per-user peers derived from the session key, **live-verified** (distinct peers per key; Niko's peer holds the real session + derived summary). Owed and recorded, not claimed: a two-live-user peer check and a recorded cross-user-recall negative |
| 21 | Agent-memory docs corrected | ✅ Non-chat sessions DO resolve a Honcho session name/peer (gateway session key → title slug → cwd basename); the old "desktop/CLI sessions carry no session key" note was wrong. User isolation rests on the sub-scoped session key, not on those sessions being peerless |

**Next in P2–3** (queued, unchanged): PWA (#11), events & notifications (#12), live capture (#48), per-trip theme presets (#40), then P4 social (#14).
